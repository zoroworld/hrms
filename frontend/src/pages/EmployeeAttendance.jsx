...
const EmployeeAttendance = () => {
  ...
  const fetchAttendance = async () => {
    try {
      ...
      if (res.data.length > 0) setEmployee(res.data[0].employee_detail);
    } catch (err) {
  ...
  return (
    <div className="container mt-4">
      <h3>
        Attendance Records{" "}
        {employee && `for ${capitalize(employee.first_name)} ${capitalize(employee.last_name)}`}
      </h3>

      <Link to="/attendance" className="btn btn-secondary mb-3">
        Back
      </Link>
